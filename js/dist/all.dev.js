...
$(document).ready(function () {
  ...
  $('.banner-list-section').on('click', function (event) {
    ...
    $(".item-card").hide();
    $(".item-card[data-title=".concat(title1, "]")).show(800);
  ...
  var cartInpuGroup = $('.cart-input-group');
  cartInpuGroup.click(function (event) {
    var i = parseInt($(event.target).parents('.cart-input-group').find('.product-qty').val());
    var price = parseInt($(event.target).parents('.cart-item').find('.product-price').text());

    switch (true) {
      case $(event.target).text() == 'add':
        //按加號時的動作
  ...
  var productQty = $('.product-qty'); //中間數量欄位的輸入時改變

  productQty.on('change', function (event) {
    var i = parseInt($(event.target).parents('.cart-input-group').find('.product-qty').val());
    var price = parseInt($(event.target).parents('.cart-item').find('.product-price').text());
  ...
  function countAll() {
    ...
    $(".product-subtotal").each(function () {
      var i = parseInt($(this).text().split(",").join(''));
      console.log(i);
      allSubtotal += i;
    });
    var allSubtotalInner = allSubtotal.toLocaleString('en-US');
